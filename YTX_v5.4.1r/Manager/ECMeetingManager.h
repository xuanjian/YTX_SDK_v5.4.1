...
/**
 ...
 @param completion      执行结果回调block
 */
-(void)listAllMultMeetingsByMeetingType:(ECMeetingType)multMeetingType andKeywords:(NSString *)keywords page:(NSInteger)page pageSize:(NSInteger)pageSize isAsc:(BOOL)isAsc completion:(void(^)(ECError *error, ECPageInfo* pageInfo, NSArray * meetingList))completion;
/**
 @brief 创建实时对讲
 @param members    成员
 @param completion 执行结果回调block
